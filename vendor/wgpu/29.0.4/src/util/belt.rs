        ...
        for chunk in self.closed_chunks.drain(..) {
            ...
            chunk
                ...
                .map_async(MapMode::Write, move |result| {
                    // Only return the chunk to the free list when the re-map
                    // SUCCEEDED. The old code sent the chunk back regardless;
                    // on the web backend device.poll(Wait) does not actually
                    // block, so a buffer still in GPU use can reject its
                    // re-map. The chunk then returned to the free list in the
                    // unmapped state and the next get_mapped_range() panicked
                    // with "Failed to execute 'getMappedRange'".
                    if result.is_ok() {
                        let _ = sender.send(chunk);
                    }
                    // On failure the chunk is dropped; allocate() creates a
                    // fresh one with mapped_at_creation: true, which is safe.
                });
        }
    }
